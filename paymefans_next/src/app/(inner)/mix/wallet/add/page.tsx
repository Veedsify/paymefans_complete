...
const WalletAddBank = () => {
    ...
    return (
        <div className="p-4 py-8">
            ...
            <div>
                <input type="text" placeholder="Account Number" onChange={AcceptNumbersOnly}
                ...
                <div
                    className="mt-2 rounded-xl p-2 flex flex-col w-full max-h-80 overflow-auto border border-primary-dark-pink">
                    {bank?.map((bank, index) => (
                        <button
                            className="font-bold text-sm block mb-1 text-left p-3 rounded border hover:bg-messages-unread bg-coins-card-bottom text-gray-700"
                            key={index}>
                            {bank.Name}
                        </button>
                    ))}
                ...
                <div className="py-5">
                    <button className={"w-full bg-primary-dark-pink py-4 text-white font-bold rounded-lg"}>
                        Set Bank Account
                    </button>
                </div>
            </div>
