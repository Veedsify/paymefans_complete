...
const BecomeAModel = () => {
    ...
    const submitData = async (e: MouseEvent<HTMLButtonElement>) => {
        ...
        if (!postAudience.name) return toast.error("Please select a gender ")
    ...
    return (
        <div className="px-4 py-6 ">
            ...
            <div>
                <div>
                    <input
                        ...
                        type="text" placeholder="First name" name="firstname" className="border mt-2 p-4 w-full rounded-lg pl-5 font-semibold outline-none" />
                </div>
                <div>
                    <input
                        onChange={updateModelSignUpData}
                        type="text" placeholder="Last name" name="lastname" className="border p-4 mt-4 w-full rounded-lg pl-5 font-semibold outline-none" />
                </div>
                <div>
                    <input
                        onChange={updateModelSignUpData}
                        type="date" placeholder="Date of Birth" name="dob" className="border p-4 mt-4 w-full rounded-lg pl-5 font-semibold outline-none" />
                </div>
                <button className="border p-4 mt-4  rounded-lg pl-5  outline-none  relative w-full">
                    ...
                    <div className={`absolute w-full left-0 mt-3 transition-all duration-300 ${dropdown ? "opacity-100 -translate-y-0 pointer-events-auto" : "opacity-0 -translate-y-5 pointer-events-none"}`}>
                ...
                <div>
                    <select onChange={updateModelSignUpData} defaultValue="1" className="border p-4 mt-4 w-full rounded-lg pl-5 font-semibold outline-none" name="country" >
                        <option defaultValue="1" disabled>
                            (--- Select Country ---)
                        </option>
                        {countries.map((location) => (
                            <option value={location.code ? location.code : ""} key={location.code}>{location.name}</option>
                        ))}
            ...
            <div className="mt-4 px">
                ...
                <label className="inline-flex cursor-pointer mt-3 items-center font-medium gap-2 text-gray-500" htmlFor="yes">
                    <input onChange={updateModelSignUpData} type="radio" name="available" id="yes" value="yes" className="accent-primary-dark-pink h-6 w-6 outline-none" />
                    Yes
                </label>
                <label className="inline-flex cursor-pointer items-center font-medium gap-2 ml-4 text-gray-500" htmlFor="no" >
                    <input onChange={updateModelSignUpData} type="radio" name="available" value="no" id="no" className="accent-primary-dark-pink h-6 w-6 outline-none" />
                    No
                    <label ></label>
                </label>
            </div>
            <p className=" mt-5 ">Pay a one time membership fee of <b>₦5,000 </b> </p>
            <button onClick={submitData} className="bg-primary-dark-pink w-full p-3 rounded-xl mt-3 mb-20 text-white font-semibold">SUBMIT & Pay</button>
        </div>
    );
}
